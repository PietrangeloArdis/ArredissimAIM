...
function App() {
  const { i18n } = useTranslation();import React, { useState, useEffect, useCallback } from 'react';
import { BrowserRouter as Router, Routes, Route, Navigate } from 'react-router-dom';
import { Navigation } from './components/Navigation';
import { Dashboard } from './components/Dashboard';
import { CampaignTable } from './components/CampaignTable';
...
import { useTranslation } from 'react-i18next';
import { updateCampaignStatuses } from './utils/updateCampaignStatuses';
import { Notification, NotificationProps } from './components/Notification'; // <-- Importa il componente
...
function DashboardApp() {
  ...
  const renderContent = () => {
    // Passa la funzione 'showNotification' ai componenti che ne hanno bisogno
    switch (activeTab) {
      case 'Dashboard':
      ...
      case 'Brands':
        return <BrandManager />; // Questi riceveranno la notifica in un passaggio successivo
      case 'Managers':
        return <ManagerManager />; // Questi riceveranno la notifica in un passaggio successivo
      case 'Channels':
        return <ChannelManager />; // Questi riceveranno la notifica in un passaggio successivo
      case 'Broadcasters':
        return <BroadcasterManager />; // Questi riceveranno la notifica in un passaggio successivo
      case 'Regions':
        return <RegionManager />; // Questi riceveranno la notifica in un passaggio successivo
      default:
